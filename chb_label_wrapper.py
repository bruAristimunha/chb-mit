...
class ChbLabelWrapper:
    ...
    def _parse_frequency(self, frequency_block):
        ...
        """
        pattern = compile_re("Data Sampling Rate: (.*?) Hz")
    ...
    def _parse_metadata(self, metadata_block, output_metadata):
        ...
        pattern_filename = compile_re("File Name: (.*?)$")
        pattern_start_time = compile_re("File Start Time: (.*?)$")
    ...
    def _parse_file_metadata(self, seizure_file_blocks):
        ...
        Note: These are not necessarily in file order, so always
        check against the filename before continuing.
